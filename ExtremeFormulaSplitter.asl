...
init
{
	...
	jit.ProcessQueue();
	vars.Resolver.Watch<uint>("ResetSave", ResetSave);
	vars.Resolver.Watch<uint>("SceneChange", SceneChange);
	...
	inst.Watch<float>("RaceTime", "RaceManager", "CurrentRaceManager", "RaceTime");						//time the race has been active, global so it continues after player finishes
	...
	vars.Resolver.WatchList<float>("LapList", funnyinst.Base, funnyinst.Offsets);
	...
	//Platforming stuff
	inst.Watch<float>("StageTime", "CharacterCamera", "Main", "CharUiRefs", "Cards", "CharInt", "ActiveTime");
	inst.Watch<bool>("PlatComplete", "CharacterCamera", "Main", "CharUiRefs", "Cards", "Complete");

	//Challenge stuff
	inst.Watch<float>("ChalTime", "ProvingGrounds", "RaceTimer");
	inst.Watch<float>("ChalCountdown", "ProvingGrounds", "CountdownTimer");
	inst.Watch<float>("ChalDone", "ProvingGrounds", "endCounter");
...
update
{
	...
	else if (current.ActiveScene == "STORY EVENT - Red Carnival World" && current.CarnivalCurrentRace == -1) {
...
split
{
	...
	else if (current.PlatComplete && current.PlatComplete != old.PlatComplete) {return true;}
	else if (current.SceneChange > old.SceneChange && current.ActiveScene != "NOMAD - ProvingGrounds Outside") {
		vars.PlatSplit = false;
		return true;
	}
	else if (settings["SplitOnCarn"] && current.CarnivalCurrentRace > old.CarnivalCurrentRace) {return true;}
...
reset
{
	...
	else if (current.ResetTourney) {return true;}
}
